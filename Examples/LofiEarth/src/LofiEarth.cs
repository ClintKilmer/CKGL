...
vec3 CalculatePointLight(PointLight light, vec3 normal, vec3 fragPosition, vec3 viewDirection, float specularStrength)
{
	...
	float specularFactor = pow(max(dot(normal, halfwayDirection), 0.0), 32.0) * IsDiffuseFactorGreaterThanZero * specularStrength * texture(TextureSpecular, vUV).r;
...
void main()
{
	...
	{
		public static Texture UVTest = Texture.Create2DFromFile("textures/Earth128.png", TextureFormat.RGBA8, TextureFilter.Nearest, TextureWrap.Repeat);
		public static Texture UVTestSpecular = Texture.Create2DFromFile("textures/Earth128Specular.png", TextureFormat.RGBA8, TextureFilter.Nearest, TextureWrap.Repeat);
	}
	#endregion
	...
	public class LofiEarth : Game
	{
		...
		{
			Engine.Init(windowTitle: "LofiEarth",
		...
		public override void Update()
		{
			...
			icosphereTransform.Rotation = Quaternion.CreateRotationZ(-0.05f) * Quaternion.CreateRotationY(Time.TotalSeconds * -0.07f);
		...
		public override void Draw()
		{
			...
			//Renderer.Draw3D.ResetTransform();
			Renderer.Draw3D.SetTransform(new Transform { Position = new Vector3(50f, 15f, -10f), Scale = new Vector3(5f) });
			Renderer.Draw3D.Cube(Colour.White);
			Renderer.Draw3D.ResetTransform();

			Textures.UVTest.Bind();
			Textures.UVTestSpecular.Bind(1);

			Shaders.PointLightShader.Bind();

			Shaders.PointLightShader.Texture = 0;
			Shaders.PointLightShader.TextureSpecular = 1;
			...
			Shaders.PointLightShader.Light1Position = new Vector3(50f, 15f, -10f) * Camera.ViewMatrix;
			Shaders.PointLightShader.Light2Position = new Vector3(1000f) * Camera.ViewMatrix;
			Shaders.PointLightShader.Light3Position = new Vector3(1000f) * Camera.ViewMatrix;
			Shaders.PointLightShader.Light1Colour = Colour.White;
			Shaders.PointLightShader.Light2Colour = light2Colour;
			Shaders.PointLightShader.Light3Colour = light3Colour;
			Shaders.PointLightShader.Light1Radius = 1000f;
			Shaders.PointLightShader.Light2Radius = 0f;
			Shaders.PointLightShader.Light3Radius = 0f;
