...
namespace CKGL
{
	public class ReverbEffect : AudioEffect
	{
		...
		public ReverbEffect(Preset preset) : this() => LoadPreset(preset);
		...
		public float Density
		{
			get
			{
				alGetEffectf(ID, AL_REVERB_DENSITY, out float value);
				Audio.CheckALError("Could not read ReverbEffect.Density");
				return value;
			}
			set
			{
				Audio.CheckRange("ReverbEffect.Density", value, AL_REVERB_MIN_DENSITY, AL_REVERB_MAX_DENSITY);
				alEffectf(ID, AL_REVERB_DENSITY, value);
				Audio.CheckALError("Could not update ReverbEffect.Density");
				Apply();
			}
		}
		...
		public float Diffusion
		{
			get
			{
				alGetEffectf(ID, AL_REVERB_DIFFUSION, out float value);
				Audio.CheckALError("Could not read ReverbEffect.Diffusion");
				return value;
			}
			set
			{
				Audio.CheckRange("ReverbEffect.Diffusion", value, AL_REVERB_MIN_DIFFUSION, AL_REVERB_MAX_DIFFUSION);
				alEffectf(ID, AL_REVERB_DIFFUSION, value);
				Audio.CheckALError("Could not update ReverbEffect.Diffusion");
				Apply();
			}
		}
		...
		public float Gain
		{
			get
			{
				alGetEffectf(ID, AL_REVERB_GAIN, out float value);
				Audio.CheckALError("Could not read ReverbEffect.Gain");
				return value;
			}
			set
			{
				Audio.CheckRange("ReverbEffect.Gain", value, AL_REVERB_MIN_GAIN, AL_REVERB_MAX_GAIN);
				alEffectf(ID, AL_REVERB_GAIN, value);
				Audio.CheckALError("Could not update ReverbEffect.Gain");
				Apply();
			}
		}
		...
		public float GainHF
		{
			get
			{
				alGetEffectf(ID, AL_REVERB_GAINHF, out float value);
				Audio.CheckALError("Could not read ReverbEffect.GainHF");
				return value;
			}
			set
			{
				Audio.CheckRange("ReverbEffect.GainHF", value, AL_REVERB_MIN_GAINHF, AL_REVERB_MAX_GAINHF);
				alEffectf(ID, AL_REVERB_GAINHF, value);
				Audio.CheckALError("Could not update ReverbEffect.GainHF");
				Apply();
			}
		}
		...
		public float DecayTime
		{
			get
			{
				alGetEffectf(ID, AL_REVERB_DECAY_TIME, out float value);
				Audio.CheckALError("Could not read ReverbEffect.DecayTime");
				return value;
			}
			set
			{
				Audio.CheckRange("ReverbEffect.DecayTime", value, AL_REVERB_MIN_DECAY_TIME, AL_REVERB_MAX_DECAY_TIME);
				alEffectf(ID, AL_REVERB_DECAY_TIME, value);
				Audio.CheckALError("Could not update ReverbEffect.DecayTime");
				Apply();
			}
		}
		...
		public float DecayHFRatio
		{
			get
			{
				alGetEffectf(ID, AL_REVERB_DECAY_HFRATIO, out float value);
				Audio.CheckALError("Could not read ReverbEffect.DecayHFRatio");
				return value;
			}
			set
			{
				Audio.CheckRange("ReverbEffect.DecayHFRatio", value, AL_REVERB_MIN_DECAY_HFRATIO, AL_REVERB_MAX_DECAY_HFRATIO);
				alEffectf(ID, AL_REVERB_DECAY_HFRATIO, value);
				Audio.CheckALError("Could not update ReverbEffect.DecayHFRatio");
				Apply();
			}
		}
		...
		public float ReflectionsGain
		{
			get
			{
				alGetEffectf(ID, AL_REVERB_REFLECTIONS_GAIN, out float value);
				Audio.CheckALError("Could not read ReverbEffect.ReflectionsGain");
				return value;
			}
			set
			{
				Audio.CheckRange("ReverbEffect.ReflectionsGain", value, AL_REVERB_MIN_REFLECTIONS_GAIN, AL_REVERB_MAX_REFLECTIONS_GAIN);
				alEffectf(ID, AL_REVERB_REFLECTIONS_GAIN, value);
				Audio.CheckALError("Could not update ReverbEffect.ReflectionsGain");
				Apply();
			}
		}
		...
		public float ReflectionsDelay
		{
			get
			{
				alGetEffectf(ID, AL_REVERB_REFLECTIONS_DELAY, out float value);
				Audio.CheckALError("Could not read ReverbEffect.ReflectionsDelay");
				return value;
			}
			set
			{
				Audio.CheckRange("ReverbEffect.ReflectionsDelay", value, AL_REVERB_MIN_REFLECTIONS_DELAY, AL_REVERB_MAX_REFLECTIONS_DELAY);
				alEffectf(ID, AL_REVERB_REFLECTIONS_DELAY, value);
				Audio.CheckALError("Could not update ReverbEffect.ReflectionsDelay");
				Apply();
			}
		}
		...
		public float LateReverbGain
		{
			get
			{
				alGetEffectf(ID, AL_REVERB_LATE_REVERB_GAIN, out float value);
				Audio.CheckALError("Could not read ReverbEffect.LateReverbGain");
				return value;
			}
			set
			{
				Audio.CheckRange("ReverbEffect.ReflectionsGain", value, AL_REVERB_MIN_LATE_REVERB_GAIN, AL_REVERB_MAX_LATE_REVERB_GAIN);
				alEffectf(ID, AL_REVERB_LATE_REVERB_GAIN, value);
				Audio.CheckALError("Could not update ReverbEffect.LateReverbGain");
				Apply();
			}
		}
		...
		public float LateReverbDelay
		{
			get
			{
				alGetEffectf(ID, AL_REVERB_LATE_REVERB_DELAY, out float value);
				Audio.CheckALError("Could not read ReverbEffect.LateReverbDelay");
				return value;
			}
			set
			{
				Audio.CheckRange("ReverbEffect.LateReverbDelay", value, AL_REVERB_MIN_LATE_REVERB_DELAY, AL_REVERB_MAX_LATE_REVERB_DELAY);
				alEffectf(ID, AL_REVERB_LATE_REVERB_DELAY, value);
				Audio.CheckALError("Could not update ReverbEffect.LateReverbDelay");
				Apply();
			}
		}
		...
		public float AirAbsorptionGainHF
		{
			get
			{
				alGetEffectf(ID, AL_REVERB_AIR_ABSORPTION_GAINHF, out float value);
				Audio.CheckALError("Could not read ReverbEffect.AirAbsorptionGainHF");
				return value;
			}
			set
			{
				Audio.CheckRange("ReverbEffect.AirAbsorptionGainHF", value, AL_REVERB_MIN_AIR_ABSORPTION_GAINHF, AL_REVERB_MAX_AIR_ABSORPTION_GAINHF);
				alEffectf(ID, AL_REVERB_AIR_ABSORPTION_GAINHF, value);
				Audio.CheckALError("Could not update ReverbEffect.AirAbsorptionGainHF");
				Apply();
			}
		}
		...
		public float RoomRolloffFactor
		{
			get
			{
				alGetEffectf(ID, AL_REVERB_ROOM_ROLLOFF_FACTOR, out float value);
				Audio.CheckALError("Could not read ReverbEffect.RoomRolloffFactor");
				return value;
			}
			set
			{
				Audio.CheckRange("ReverbEffect.RoomRolloffFactor", value, AL_REVERB_MIN_ROOM_ROLLOFF_FACTOR, AL_REVERB_MAX_ROOM_ROLLOFF_FACTOR);
				alEffectf(ID, AL_REVERB_ROOM_ROLLOFF_FACTOR, value);
				Audio.CheckALError("Could not update ReverbEffect.RoomRolloffFactor");
				Apply();
			}
		}
		...
		public bool DecayHFLimit
		{
			get
			{
				alGetEffecti(ID, AL_REVERB_DECAY_HFLIMIT, out int decayHFLimit);
				Audio.CheckALError("Could not read ReverbEffect.DecayHFLimit");
				return decayHFLimit == 1;
			}
			set
			{
				alEffecti(ID, AL_REVERB_DECAY_HFLIMIT, value ? 1 : 0);
				Audio.CheckALError("Could not update ReverbEffect.DecayHFLimit");
				Apply();
			}
		}
		...
		{
			alEffectf(ID, AL_REVERB_DENSITY, preset.Density);
			Audio.CheckALError("Could not update ReverbEffect.Density");

			alEffectf(ID, AL_REVERB_DIFFUSION, preset.Diffusion);
			Audio.CheckALError("Could not update ReverbEffect.Diffusion");

			alEffectf(ID, AL_REVERB_GAIN, preset.Gain);
			Audio.CheckALError("Could not update ReverbEffect.Gain");

			alEffectf(ID, AL_REVERB_GAINHF, preset.GainHF);
			Audio.CheckALError("Could not update ReverbEffect.GainHF");

			alEffectf(ID, AL_REVERB_DECAY_TIME, preset.DecayTime);
			Audio.CheckALError("Could not update ReverbEffect.DecayTime");

			alEffectf(ID, AL_REVERB_DECAY_HFRATIO, preset.DecayHFRatio);
			Audio.CheckALError("Could not update ReverbEffect.DecayHFRatio");

			alEffectf(ID, AL_REVERB_REFLECTIONS_GAIN, preset.ReflectionsGain);
			Audio.CheckALError("Could not update ReverbEffect.ReflectionsGain");

			alEffectf(ID, AL_REVERB_REFLECTIONS_DELAY, preset.ReflectionsDelay);
			Audio.CheckALError("Could not update ReverbEffect.ReflectionsDelay");

			alEffectf(ID, AL_REVERB_LATE_REVERB_GAIN, preset.LateReverbGain);
			Audio.CheckALError("Could not update ReverbEffect.LateReverbGain");

			alEffectf(ID, AL_REVERB_LATE_REVERB_DELAY, preset.LateReverbDelay);
			Audio.CheckALError("Could not update ReverbEffect.LateReverbDelay");

			alEffectf(ID, AL_REVERB_AIR_ABSORPTION_GAINHF, preset.AirAbsorptionGainHF);
			Audio.CheckALError("Could not update ReverbEffect.AirAbsorptionGainHF");

			alEffectf(ID, AL_REVERB_ROOM_ROLLOFF_FACTOR, preset.RoomRolloffFactor);
			Audio.CheckALError("Could not update ReverbEffect.RoomRolloffFactor");

			alEffecti(ID, AL_REVERB_DECAY_HFLIMIT, preset.DecayHFLimit ? 1 : 0);
			Audio.CheckALError("Could not update ReverbEffect.DecayHFLimit");
